...
import { Scale, HeartHandshake, ShieldAlert, ExternalLink, BookOpen, AlertTriangle } from 'lucide-react';
import clsx from 'clsx';
import QRCode from 'qrcode';

// ─────────────────────────────────────────────────────────────────────────────
// Per-scenario verified primary sources (researched & confirmed live)
// ─────────────────────────────────────────────────────────────────────────────
const SCENARIO_SOURCES: Record<string, {
  ...
  'signature-strike': {
    ...
    org: 'Bureau of Investigative Journalism',
    year: '2008–2015',
...
const UN_CCW_URL = 'https://disarmament.unoda.org/the-convention-on-certain-conventional-weapons/background-on-laws-in-the-ccw/';
        ...
        <div className="p-4 space-y-3 overflow-y-auto flex-1">
                    ...
                    className={clsx(
                      ...
                      row.type.toLowerCase().includes('wound')
                        ...
                        : row.type.toLowerCase().includes('confirmed combatant') ||
                          row.type.toLowerCase().includes('neutralized')
                        ? 'text-terminal-green'
                        : 'text-terminal-text'
                    )}
                  >
          ...
          {source && (
            <div className="border border-terminal-amber/40 rounded bg-terminal-amber-dim/10 p-3 space-y-1.5">
              ...
              <div className="text-[10.5px] text-terminal-text font-bold leading-snug">
                {source.label}
              </div>
              <div className="text-[9px] text-terminal-text-dim">
                <span className="text-terminal-amber font-bold">{source.org}</span>
                {' '}·{' '}
          ...
          {activeScenario.legalContext && (
            <div className="border border-terminal-blue/30 rounded p-3 bg-terminal-blue-dim/10 text-[10px] leading-relaxed space-y-1.5">
              ...
              <div>
                ...
                >
                  [UN CCW — LAWS <ExternalLink className="w-2 h-2" />]
                </a>
              </div>
            ...
            <div className="flex-1 space-y-1.5 text-[10px]">
              ...
              <div className="flex flex-wrap gap-2 pt-0.5">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1 text-terminal-blue hover:underline font-bold"
                >
                  UN CCW Negotiations <ExternalLink className="w-2.5 h-2.5" />
                </a>
              </div>
            </div>
            {/* QR → petition */}
          ...
          <div className="bg-terminal-red-dim/10 border border-terminal-red/30 rounded p-2.5 flex items-start gap-2 text-[9px]">
            ...
            <p className="text-terminal-text-dim leading-relaxed">
              ...
              for the outcome. This is an explicit violation of the principle of{' '}
